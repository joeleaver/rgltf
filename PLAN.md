...
| **5** ✅ | Animation + skinning (LINEAR/STEP/CUBICSPLINE, joints), `EXT_mesh_gpu_instancing`, morph targets. | **DONE** — 5a animation (BoxAnimated), 5b GPU skinning (RiggedSimple/CesiumMan/Fox + storage-buffer joint palette), 5c `EXT_mesh_gpu_instancing` (per-instance TRS; verified against a `gltfpack -mi -c` asset), 5d morph targets (CPU morphing, AnimatedMorphCube). App owns the winit loop (game-embed model) for a continuous render loop. Each sub-phase adversarially reviewed (0 confirmed defects) + verified via the rinch MCP bridge. |
| **6** ✅ | IBL, material extensions polish, camera presets, drag-and-drop open, perf. | **DONE** — split-sum IBL (prefiltered specular + irradiance + BRDF LUT) from a procedural environment + analytic skybox; `KHR_materials_ior`/`specular`/`clearcoat`/`sheen` + `transmission`/`volume` (two-pass screen-space refraction with roughness blur + Beer-Lambert absorption); camera view presets + reset; drag-and-drop open; large-model fix (adapter limits); skeleton overlay + bone-weights debug views. Each sub-phase adversarially reviewed (multi-lens, refute-by-default) + verified live vs Khronos conformance assets via the rinch MCP bridge. Deferred polish: transmission/thickness *textures* (factor-driven), HDR-deferred tone-mapping (transmission double-aces), formal large-model perf profiling. |
| **7** ✅ | Productize for Linux: licensing, release hygiene, app icon, desktop/MIME integration, and distributable packages. | **DONE** — dual `LICENSE-MIT`/`LICENSE-APACHE`; the rinch MCP debug bridge is gated behind a default-off `debug` cargo feature, so `cargo build --release` and every package omit the listening socket (verified: no `rinch-debug` symbols in the release binary); a wireframe→shaded-cube app icon (`assets/branding/rgltf.svg` master + hicolor PNGs 16–512); `assets/linux/rgltf.desktop` (passes `desktop-file-validate`) + a shared-mime-info XML registering `model/gltf-binary`/`model/gltf+json` → `.glb`/`.gltf` get an *Open With ▸ rgltf* handler (verified via `mimeinfo.cache`); the window sets its Wayland `app_id` / X11 `WM_CLASS` to `rgltf` to bind the installed icon. Packages: `cargo deb -p rgltf-app` → `rgltf_0.1.0-1_amd64.deb` (FHS layout, `libvulkan1` dep, postinst/postrm cache refresh) and `assets/linux/appimage/build-appimage.sh` → `rgltf-x86_64.AppImage` (both verified to build; render path re-verified live post-edit; AppImage launches clean). Not yet: Flatpak & AUR, a signed GitHub release. |
